...
void main() {
  group('Pipeline Integration Tests (Phase 10 Update)', () {
    ...
    test('Geometry + Filter: rotateRight -> flipH -> grayscale', () async {
      ...
      image.getPixel(0, 0).setRgb(255, 0, 0); // Red pixel
      final originalBytes = Uint8List.fromList(img.encodeJpg(image));
      ...
      final resultImg = img.decodeImage(result.bytes)!;
    ...
    test('Edge + Threshold: Sobel -> Threshold produces binary output', () async {
      ...
      // Create an edge
      for(int x=0; x<5; x++) for(int y=0; y<10; y++) image.getPixel(x, y).setRgb(0, 0, 0);
      for(int x=5; x<10; x++) for(int y=0; y<10; y++) image.getPixel(x, y).setRgb(255, 255, 255);
    ...
    test('Histogram Analysis reflects processed image state', () async {
      ...
      // 2. Analyze histogram
      final stats = await HistogramAnalyzer.analyze(result.bytes);
